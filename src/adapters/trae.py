...
### Key: Getting the SESSION_ID

SESSION_ID is auto-generated by the SessionStart hook at session launch, format: `2026-04-06_HHMMSS` (e.g. `2026-04-06_143025`).

**How to find it (priority order)**:
1. Check session start system output for `[LogRelay] SESSION_ID=` line — the value after `=` is the SESSION_ID
2. If not found, search for active log files: `grep -rl "status: active" logs/trae/ 2>/dev/null` — extract SESSION_ID from the filename (first 17 chars of `2026-04-06_HHMMSS-xxx.md`)
3. If still not found, generate from current time: `2026-04-06_HHMMSS`

### Steps

1. Review all work done in this session
2. Get the SESSION_ID using the method above
3. Generate structured summary (JSON format):
   ...
   {{
     ...
     "handoff_to": null,
     "conversation": "Full conversation timeline (must include timestamps, what user said, what assistant did)"
   }}
   ```

   **conversation field requirements**:
   - Must be a timeline-formatted conversation record, NOT a one-line summary
   - Format example: "### [14:30] User: Optimize PPT speech\\n### [14:31] Assistant: Read meeting notes...\\n### [14:35] User: Use plain language\\n### [14:36] Assistant: Updated Page 1-7..."

4. Run command:
   `{python_cmd} "{scripts_root}/src/hooks/session_end.py" --tool trae --session-id <SESSION_ID> --data '<JSON data>'

5. If command reports "active log file not found", the SessionStart hook may not have fired. As fallback, write the log directly:
   - Create log file in `logs/trae/` directory (create dir if needed)
   - Filename format: `SESSION_ID-summary-keyword.md`
   - Write full log with frontmatter + summary layer + %% + conversation layer
   - Also update `logs/STATUS.md`
